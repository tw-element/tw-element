...
describe('menu', () => {
  ...
  test('background-color', async () => {
    ...
    expect(
      window.getComputedStyle(instance)._values['--tw-menu-bg-color']
  ...
  test('open method', async () => {
    ...
    const TwSubMenu = wrapper.findComponent({ ref: 'subMenu' })
    expect(TwSubMenu.vm.$.exposed.opened).toBeTruthy()
  })
